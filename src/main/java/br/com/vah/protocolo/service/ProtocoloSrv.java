...
public class ProtocoloSrv extends AbstractSrv<Protocolo> {
	...
	private @Inject PrescricaoMedicaSrv prescricaoMedicaSrv;

	private @Inject AvisoCirurgiaSrv avisoCirurgiaSrv;

	private @Inject RegistroDocumentoSrv registroDocumentoSrv;

	private @Inject ItemProtocoloSrv itemProtocoloSrv;

	private @Inject RegFaturamentoSrv regFaturamentoSrv;

	private @Inject EvolucaoEnfermagemSrv evolucaoEnfermagemSrv;

	private @Inject CaixaEntradaSrv caixaSrv;

	private @Inject ValidadeSrv validadeSrv;

	private @Inject HDASrv hdaSrv;

	private @Inject ClassificacaoDeRiscoSrv classSrv;
	...
	public StreamedContent relatorioTotalPorSetor(Date inicio, Date termino, List<Setor> setores, String[] relations) {
		...
		for (Protocolo Protocolo : Protocolos) {
			...
			if (dataResp != null) {
				...
				report.setTempoMedioRecebimento(
						report.getTempoMedioRecebimento() + Float.valueOf(diasResp / report.getTotalCriadas()));
	...
	public Protocolo addHistorico(Protocolo protocolo, User user, SetorProtocolo origem, SetorProtocolo destino,
			String descricao, AcaoHistoricoEnum acao) {
		if (user != null) {
	...
	private Map<String, Object> buscarDocumentos(Protocolo protocolo, Date begin, Date end, Convenio convenio,
			String listaContas) {
		...
		if (SetorNivelEnum.SECRETARIA.equals(nivelOrigem)) {
			...
			final List<ClassificacaoDeRisco> classificacao = new ArrayList<>();
			List<DocumentoProtocolo> documentosPs = new ArrayList<>();
			
			/**/
			List<Object[]> datas = new ArrayList<>();
			if (!protocolo.getAtendimento().getTipo().equalsIgnoreCase("U")) {
				...
				obj[0] = begin;
				obj[1] = begin;
				obj[2] = new BigDecimal(26);
				obj[3] = end;
				datas.add(obj);
			}
			datas.forEach((range) -> {
				prescricoes.addAll(prescricaoMedicaSrv.consultarPrescricoes(protocolo, (Date) range[0], (Date) range[1],
						(Date) range[3]));
				// TODO: Colocar checagem de folhas anestésicas (Tabelas
				// PRESTADOR_AVISO, ATI_MED, PRESTADOR e CIRURGIA_AVISO)
				/*
				 * SELECT DISTINCT 'X' FROM DBAMV.PRESTADOR_AVISO
				 * PRESTADOR_AVISO, DBAMV.ATI_MED ATI_MED, DBAMV.PRESTADOR
				 * PRESTADOR, DBAMV.CIRURGIA_AVISO CIRURGIA_AVISO WHERE
				 * (PRESTADOR_AVISO.CD_PRESTADOR = PRESTADOR.CD_PRESTADOR AND
				 * PRESTADOR_AVISO.CD_ATI_MED = ATI_MED.CD_ATI_MED AND TP_FUNCAO
				 * = 'N' AND CIRURGIA_AVISO.CD_AVISO_CIRURGIA =
				 * PRESTADOR_AVISO.CD_AVISO_CIRURGIA AND
				...
				registros.addAll(
						registroDocumentoSrv.consultarRegistros(protocolo, inicioValidade, fimValidade, referencia));
				evolucoes.addAll(evolucaoEnfermagemSrv.consultarEvolucoesEnfermagem(protocolo, inicioValidade,
						fimValidade, referencia));

				/**/
				if (protocolo.getAtendimento().getTipo().equalsIgnoreCase("U")
						&& protocolo.getAtendimento().getDataAlta() != null) {
					hdas.addAll(hdaSrv.consultarHdas(protocolo, inicioValidade, fimValidade, referencia));
					classificacao.addAll(
							classSrv.consultarClassificacao(protocolo, inicioValidade, fimValidade, referencia));

					// if (!hdas.isEmpty() && hdas.size() > 0){
					// Folha de Diagnostico
					// Guia SADT (há restricao de convenio)
			...
			documentosPs = hdaSrv.diagnosticoPs(hdas);
			documentosPs.addAll(classSrv.guiaSadt(hdas));
			...
			classificacao.forEach((c) -> rawResult.add(new DocumentoDTO(c)));
			documentosPs.forEach((dp) -> rawResult.add(new DocumentoDTO(dp)));
		...
		// Busque itens na caixa de entrada (documentos repassados, protocolos
		// recebidos)
	...
	public DtoKeyMap buscarDocumentosNaoSelecionados(Protocolo protocolo, Date begin, Date end, Convenio convenio,
			String listaContas) throws ProtocoloBusinessException {
		if (SetorNivelEnum.SECRETARIA.equals(protocolo.getOrigem().getNivel())) {
			if (begin == null) {
				...
				Map<String, Object> resultado = buscarDocumentos(protocolo, beginCld.getTime(), endCld.getTime(),
						convenio, listaContas);
	...
	public Protocolo enviarProtocolo(final Protocolo protocolo, User user, List<CaixaEntrada> caixasVinculadas,
			List<CaixaEntrada> caixasRemovidas) throws ProtocoloBusinessException {
		...
		// Verifique se há documentos pendentes caso a origem do envio seja uma
		// secretaria
		if (SetorNivelEnum.SECRETARIA.equals(nivelDestino)) {
			...
			if (conta != null) {
				List<CaixaEntrada> candidatos = caixaSrv.busqueDocumentosNaoVinculados(protocolo.getAtendimento(),
						origem);
				...
				if (candidatos.size() > seraoEnviados.size()) {
					throw new ProtocoloBusinessException(
							"O envio não foi realizado, pois existem documentos pendentes de inclusão.");
				}
			}
		}
		...
		if (SetorNivelEnum.FATURAMENTO_CENTRAL.equals(nivelOrigem)) {
			// Se o destino for uma secretaria, verifica se há mais de uma conta
		...
		if (EstadosProtocoloEnum.RASCUNHO.equals(protocolo.getEstado())
				|| EstadosProtocoloEnum.RECUSADO.equals(protocolo.getEstado())) {
			return salvarProtocolo(protocolo, user, AcaoHistoricoEnum.MOVIMENTO, caixasVinculadas, caixasRemovidas);
		} else {
			throw new ProtocoloBusinessException(
					"Apenas rascunhos, protocolos recusados ou novos protocolos podem ser enviados.");
		}
	}
	...
	public Integer[] contarDocumentos(Protocolo protocolo) {
		...
		return new Integer[] { totalDocumentos, totalPrescricoes, totalEvolucoes, totalFolha, totalDescricoes,
				totalRegistros, totalDocumentosManuais, totalEvolucaoAnotacao, totalDocumentosProntuarios };
	}
	...
	public Protocolo salvarProtocolo(Protocolo protocolo, User user, AcaoHistoricoEnum acao,
			List<CaixaEntrada> caixasVinculadas, List<CaixaEntrada> caixasRemovidas) {
		...
		if (AcaoHistoricoEnum.RECEBIMENTO.equals(acao)) {
			...
			if (SetorNivelEnum.SECRETARIA.equals(nivelOrigem)) {
				if (!SetorNivelEnum.SECRETARIA.equals(nivelDestino)) {
					// se estiver enviando para um setor de um nível acima
					// (exemplo: Faturamento, Envios), crie somente um item na
					// caixa de entrada.
					caixaSrv.create(caixaSrv.criarCaixa(protocolo));
				} else {
					// Se estiver transferindo para outra secretaria, crie itens
					// de caixa para cada documento.
					protocolo.getItens()
							.forEach((filho) -> caixaSrv.create(caixaSrv.criarCaixa(protocolo, filho.getDocumento())));
				}
				...
				protocolo.getItens().forEach((item) -> {
					...
					// Altera a origem e o destino para coincidir com o
					// protocolo que está sendo recebido.
					// Isto é necessário, pois o método "criarCaixa" utiliza
					// origem/destino do protocolo passado como parâmetro.
